...
@with_kw struct Model_Parameters @deftype Float64
    ...
    ζ_grid::Vector{Float64} = rouwenhorst(σ_ζ/(1 - φ^2), φ, 3)[1]
    T_ζ::Matrix{Float64} = rouwenhorst(σ_ζ/(1 - φ^2), φ, 3)[2]
    ...
    κ::Matrix{Float64} = vcat(hcat(CSV.File("life_cycle_income.csv").age,CSV.File("life_cycle_income.csv").deterministic_component),[56.0 0.0])
    
    # Punishment for defaulting 
    pun::Float64 = -10^8 

    # Cash-on-Hand grids
    nX::Int64 = 500 # Number of cash on hand grid points 
    ...
    nM::Int64 = 25 # Number of mortgage debt grid points 
...
function build_solutions(para) 
    ...
    for j = 1:para.N + 1
        X_min = para.rent_prop * para.P_bar * exp(para.b * (j-1))
        X_max = 70000 * (1.08)^j  # Maximum cash on hand allowed
        X_grids[:,j] = collect(range(X_min, length = para.nX, stop = X_max)) 
    end 
